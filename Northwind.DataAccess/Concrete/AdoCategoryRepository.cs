...
namespace Northwind.DataAccess.Concrete
{
    public class AdoCategoryRepository : ICategoryRepository
    {
        ...
        public async Task<Category> GetByIdAsync(int id)
        {
            ...
            using (var client = new HttpClient(httpClientHandler))
            {
                var response = client.AddHeader().GetAsync("/api/categories/" + id).Result;
        ...
        public async Task<Category> InsertAsync(Category entity)
        {
            ...
            using (var httpClient = new HttpClient(httpClientHandler))
            {
                ...
                HttpResponseMessage response = await httpClient.AddHeader().PostAsync("/api/categories", contentData);
                var result = JsonConvert.DeserializeObject<Category>(await response.Content.ReadAsStringAsync());
        ...
        public async Task<Category> UpdateAsync(Category entity)
        {
            ...
            using (var httpClient = new HttpClient(httpClientHandler))
            {
                ...
                HttpResponseMessage response = await httpClient.AddHeader().PutAsync("/api/categories", contentData);
                var result = JsonConvert.DeserializeObject<Category>(await response.Content.ReadAsStringAsync());
